...
```{r overtimegraphic, echo = FALSE, out.width = "40%", fig.align = "middle"}
...
```{r q1p1, out.width="100%", fig.width=6, fig.height=20, echo = FALSE}
ggplot(q1_p1_data %>% filter(year %in% c(2000, 2005, 2010,2015, 2020)), aes(type, value, fill  = type)) +
  geom_col() + labs(x = 'Ad Type', y= " Ad Count", fill = "Ad Characteristic") + theme_minimal() + theme( axis.text.x=element_blank(), axis.title.x = element_blank(), legend.position = c(0.8, 0.24), legend.key.size = unit(.5, 'cm'),
        legend.key.height = unit(.5, 'cm'), #change legend key height
        legend.key.width = unit(.5, 'cm'), legend.background = element_rect(size = 0.5, colour = 1)) + scale_fill_brewer(palette = "Spectral") + facet_wrap(~year, ncol =3) + scale_y_continuous(breaks = seq(0, 12, by = 2))
```
...
- considerable changes over time in
the content of Super Bowl ads
- use of sex has dropped since 2015: why?
- funny and showing the product quickly consistently high: why?
- surprising finding: patriotism very low majority of years
...
ggplot(q1_p2_data2, aes(fill=type, y=per, x=brand_type)) + 
  ...
  labs(x= "Brand Type",
       y = "Percentage", fill = "Ad Features") + geom_bar(position="stack", stat="identity", color = "black") + coord_flip() + scale_fill_brewer(palette = "Spectral") + theme_minimal() + labs(x= "Brand Type", y = "Percentage", fill = "Ad Features") +
...
- NFL owned ads feature celebrities at a much higher rate compared to other
brand types: why?
- Consistent with last plot: other brand types have features funny and shows product quickly as most prevalent
